...
class Parser:
    ...
    def parse(self) -> DroneMap:
        ...
        if nb_drones is None:
            raise ParseError(1, f"{RED}missing 'nb_drones' declaration{RESET}")
        ...
        if drone_map.start.x == drone_map.end.x and drone_map.start.y == drone_map.end.y:
            raise ParseError(0, f"{RED}start_hub and end_hub cannot share the same coordinates{RESET}")
    ...
    def _parse_nb_drones(self, line: str, line_no: int) -> int:
        try:
            ...
            raise ParseError(line_no, f"{RED}invalid nb_drones value{RESET}")
        if nb <= 0:
            raise ParseError(line_no, f"{RED}nb_drones must be a positive integer{RESET}")
        return nb
    ...
    def _parse_zone(
        ...
        if "[" in rest:
            ...
            if not metadata_part.strip().endswith("]"):
                raise ParseError(line_no, f"{RED}malformed metadata block, missing ']'{RESET}")
            metadata_str = metadata_part.strip()[:-1]
            duplicate_check = metadata_str.split()
            pile = []
            for token in duplicate_check:
                if "=" not in token:
                    raise ParseError(line_no, f"{RED}invalid metadata token '{token}'{RESET}")
                key, value = token.split("=")
                if key not in ["zone", "color", "max_drones"]:
                    raise ParseError(line_no, f"{RED}invalid metadata key '{key}'{RESET}")
                pile.append(key)
                if value == "":
                    raise ParseError(line_no, f"{RED}metadata key '{key}' has empty value{RESET}")
                if key == "color" and value not in COLORS:
                    raise ParseError(line_no, f"{RED}invalid color value '{value}'{RESET}")
            if len(pile) != len(set(pile)):
                raise ParseError(line_no, f"{RED}duplicate metadata keys in zone definition{RESET}")


        parts = rest.strip().split()
        if len(parts) != 3:
            raise ParseError(line_no, f"{RED}expected '<name> <x> <y>', got '{rest.strip()}'{RESET}")

        name, x_str, y_str = parts

        if "-" in name or " " in name:
            raise ParseError(line_no, f"{RED}invalid zone name '{name}' (no dashes/spaces allowed){RESET}")
        if name in drone_map.zones:
            raise ParseError(line_no, f"{RED}duplicate zone name '{name}'{RESET}")

        try:
            x, y = int(x_str), int(y_str)
        except ValueError:
            raise ParseError(line_no, f"{RED}zone coordinates must be integers{RESET}")

        metadata = self._parse_metadata(metadata_str, line_no)

        zone_type = metadata.get("zone", "normal")
        if zone_type not in VALID_ZONE_TYPES:
            raise ParseError(line_no, f"{RED}invalid zone type '{zone_type}'{RESET}")
    ...
    def _parse_connection(self, line: str, line_no: int, drone_map: DroneMap) -> None:
        ...
        if len(names) != 2:
            raise ParseError(line_no, f"{RED}invalid connection syntax: '{rest.strip()}'{RESET}")

        name_a, name_b = names[0].strip(), names[1].strip()

        if name_a not in drone_map.zones or name_b not in drone_map.zones:
            raise ParseError(line_no, f"{RED}connection references undefined zone(s): {name_a}-{name_b}{RESET}")
        ...
        for existing in drone_map.connections:
            if {existing.zone_a.name, existing.zone_b.name} == {name_a, name_b}:
                raise ParseError(line_no, f"{RED}duplicate connection '{name_a}-{name_b}'{RESET}")
    ...
    def _parse_positive_int(self, value: str, line_no: int, field_name: str) -> int:
        try:
            ...
            raise ParseError(line_no, f"{RED}'{field_name}' must be an integer{RESET}")
        if n <= 0:
            raise ParseError(line_no, f"{RED}'{field_name}' must be a positive integer{RESET}")
        return n
